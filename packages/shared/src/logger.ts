...
const getColor = (
  ...
  switch (fgcKey) {
    ...
    case "magenta":
    case "cyan":
      fgc = isSuppers256Color
        ? friendlyColors.brightFgc[fgcKey]
        : friendlyColors.fgc[fgcKey];
      break;
  }
...
export class Logger {
  ...
  print(
    ...
    if (this.#prefix || this.#name) {
      const prefixStr = this.#prefix
        ? ` ${getScoped(this.#prefix, true)} ${this.#prefix} ${getColorReset()}`
        : "";
      const nameStr = this.#name
        ? `${getScoped(this.#name)} [${this.#name}] ${getColorReset()}`
        : "";
      const message = strings.join("");
      console.log(
        `${icon}${prefixStr}${nameStr}${getScoped(this.#name) === getColor(foregroundColor, "", true) ? getColor("blue", backgroundColor) : getColor(foregroundColor, backgroundColor)}${message}${getColorReset()}`
      );
      return;
    }

    console.log(
      `${icon}${getColor(foregroundColor, backgroundColor)}${strings.join("")}${getColorReset()}`
    );
  }
  ...
  log(message: string, useIcon?: boolean) {
    const icon =
      useIcon === false ? "" : useIcon || this.#useIcon ? "\u25ce " : "";
